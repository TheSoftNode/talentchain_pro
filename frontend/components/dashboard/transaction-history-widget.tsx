...
export function TransactionHistoryWidget({ className }: TransactionHistoryWidgetProps) {
  ...
  return (
    ...
    >
      {/* Mobile stats panel */}
      <div className="lg:hidden bg-slate-50 dark:bg-slate-900/80 rounded-lg p-3 mb-4">
      ...
      {isLoading && transactions.length === 0 ? (
        <div className="space-y-4">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="bg-slate-50 dark:bg-slate-900/80 rounded-lg p-4 animate-pulse">
              {/* <div className="flex items-center space-x-4">
                <div className="w-10 h-10 bg-slate-200 dark:bg-slate-900/80 rounded-lg flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="h-4 bg-slate-200 dark:bg-slate-900/80 rounded w-full max-w-xs mb-2" />
                  <div className="h-3 bg-slate-100 dark:bg-slate-900/80 rounded w-32" />
                </div>
                <div className="w-16 h-6 bg-slate-200 dark:bg-slate-900/80 rounded flex-shrink-0" />
              </div> */}
            </div>
          ))}
        </div>
          ...
          <AnimatePresence mode="popLayout">
            {filteredTransactions.map((transaction, index) => {
              ...
              return (
                ...
                >
                  <Card className="group hover:shadow-lg dark:bg-slate-900/80 hover:border-hedera-300 dark:hover:border-hedera-600 transition-all duration-300">
